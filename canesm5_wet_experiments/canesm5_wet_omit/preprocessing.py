...
def main():
    ...
    mask = os.path.join(home, root, experiment, 'data/canesm5_wet_omit_mask.npy')
...
def gen_mask(path, mask, save='off'):
    ...
    print(f'index for omitting: {omit_extent}')
...
def show(path):
    ...
    tp = ax.imshow(img, cmap=jmacmap() ,origin='lower',
